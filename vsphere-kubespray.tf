...
data "template_file" "keepalived_master" {
  ...
  vars = {
    net_interface = "${var.vm_haproxy_net_interface}"
...
resource "null_resource" "haproxy_install" {
  ...
  provisioner "local-exec" {
    command = "cd ansible/haproxy && ansible-playbook -i ../../config/hosts.ini -b -u ${var.vm_user} -e \"ansible_ssh_pass=$VM_PASSWORD ansible_become_pass=$VM_PRIVILEGE_PASSWORD\" ${lookup(local.extra_args, var.vm_distro)} -v haproxy.yml -vvvv"
